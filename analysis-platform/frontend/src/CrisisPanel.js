...
// 국내 사이버 위기 경보단계 — KrCERT 실시간(백엔드 /api/crisis-level)
...
const pt = (r, deg) => {
  ...
  return [CX + r * Math.cos(a), CY - r * Math.sin(a)]; // y는 위로
};
const sector = (a1, a2) => {
  const [ox1, oy1] = pt(R_OUT, a1);
  const [ox2, oy2] = pt(R_OUT, a2);
  const [ix2, iy2] = pt(R_IN, a2);
  const [ix1, iy1] = pt(R_IN, a1);
  return `M ${ox1} ${oy1} A ${R_OUT} ${R_OUT} 0 0 1 ${ox2} ${oy2} `
       + `L ${ix2} ${iy2} A ${R_IN} ${R_IN} 0 0 0 ${ix1} ${iy1} Z`;
};
...
export default function CrisisPanel() {
  ...
  return (
    <div className={"crisis-panel" + (open ? "" : " is-collapsed")}>
      <div className="crisis-head-toggle" onClick={() => setOpen((o) => !o)}>
        <span className="crisis-title">사이버 위기 경보 단계</span>
      ...
      <svg className="crisis-gauge-svg" viewBox="0 0 300 168">
        <defs>
          <filter id="crisisGlow" x="-30%" y="-30%" width="160%" height="160%">
        ...
        {ORDER.map((lv, i) => {
          ...
          return (
            <path
              ...
              stroke={active ? "#fff" : "rgba(8,18,28,0.85)"}
              strokeWidth={active ? 3 : 1}
              filter={active ? "url(#crisisGlow)" : undefined}
            />
          );
        })}
        {ORDER.map((lv, i) => {
      ...
      <div className="crisis-now">
        ...
        <span className="crisis-eng">({d.eng})</span> · {d.desc}
      </div>
      <div className="crisis-foot">
        {d.notice}
        {d.stale ? " · (오프라인 폴백)" : ""} · 출처 KrCERT
      </div>
      </div></div>
    </div>
  );
